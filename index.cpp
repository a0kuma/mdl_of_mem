...
public:
                         ...
                         Computation_description computation_description)
    {
        ...
        if (computation_description == Computation_description::forward)
        {
            ...
            {
                sockets[Uio::d_i_minus_1] = ComputationIoSocket(Uio::d_i_minus_1, compute_IO_type, computation_description);
                sockets[Uio::weight] = ComputationIoSocket(Uio::weight, compute_IO_type, computation_description);
            }
            else if (compute_IO_type == Compute_IO_type::output)
            {
                sockets[Uio::d_i] = ComputationIoSocket(Uio::d_i, compute_IO_type, computation_description);
            }
            else
            {
        ...
        else if (computation_description == Computation_description::backward)
        {
            if (compute_IO_type == Compute_IO_type::input)
            {
                sockets[Uio::partial_L_over_partial_d_i] = ComputationIoSocket(Uio::partial_L_over_partial_d_i, compute_IO_type, computation_description);
                sockets[Uio::d_i_minus_1] = ComputationIoSocket(Uio::d_i_minus_1, compute_IO_type, computation_description);
                sockets[Uio::weight] = ComputationIoSocket(Uio::weight, compute_IO_type, computation_description);
            }
            else if (compute_IO_type == Compute_IO_type::output)
            {
                sockets[Uio::partial_L_over_partial_d_i_minus_1] = ComputationIoSocket(Uio::partial_L_over_partial_d_i_minus_1, compute_IO_type, computation_description);
                sockets[Uio::partial_L_over_partial_weight] = ComputationIoSocket(Uio::partial_L_over_partial_weight, compute_IO_type, computation_description);
            }
            else
            {
                throw invalid_argument(STRING_FUCKED);
...
public:
    ...
    Computation(Computation_description computation_description)
    {
        ...
        {
        forward:
            io_sockets[Compute_IO_type::input] = ComputationIoSockets(Compute_IO_type::input, computation_description);
            io_sockets[Compute_IO_type::output] = ComputationIoSockets(Compute_IO_type::output, computation_description);
            break;
        backward:
            io_sockets[Compute_IO_type::input] = ComputationIoSockets(Compute_IO_type::input, computation_description);
            io_sockets[Compute_IO_type::output] = ComputationIoSockets(Compute_IO_type::output, computation_description);
